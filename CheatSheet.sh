...
awk -F',' 'BEGIN { IGNORECASE = 1 } NR == 1 || $1 ~ /password/ { print }' Drive_C.csv

# Search files with interesting extensions
# Sysadmin files
admin=$(awk -F',' 'BEGIN { IGNORECASE = 1 } NR == 1 || ($5 !~ /^"C:\\Windows\\WinSxS\\/ && $1 ~ /\.(kdbx?|pfx|p12|pem|p7b|key|csr|keystore|keys|jks|ppk|crt|cer|pub|config|cfg|conf|ini|sln|\w{2}proj|sql|cmd|bat|ps1|vbs|log|rdp|rdg|ica|ovpn|pbk|wim|vhdx?|vmdk)"$/ && $1 !~ /"desktop.ini"/ && $3 != "\"0\"") { print }' Drive_C.csv)
awk -F',' '$2 !~ "d....." { split($1, a, /[.]/); ext = a[length(a)]; gsub(/"/,"",ext); count[ext]++ } END { for (e in count) printf ".%s\t%d\n", e, count[e] | "sort -k2nr" }' <<< $admin | head -n25 | column -t -s,

# Office files
office=$(awk -F',' 'BEGIN { IGNORECASE = 1 } NR == 1 || $1 ~ /\.((doc|xls|ppt|pps|vsd)[xm]?|txt|csv|one|pst|url|lnk)"$/ { print }' Drive_C.csv)
awk -F',' '$2 !~ "d....." { split($1, a, /[.]/); ext = a[length(a)]; gsub(/"/,"",ext); count[ext]++ } END { for (e in count) printf ".%s\t%d\n", e, count[e] | "sort -k2nr" }' <<< $office | head -n25 | column -t -s,

# List .cmd extensions that are not in C:\Windows\WinSxS
awk -F',' 'BEGIN { IGNORECASE = 1 } NR == 1 || $1 ~ /\.cmd"$/ && $5 !~ /^"C:\\Windows\\WinSxS\\/ { print }' Drive_C.csv | column -t -s,
